...
class PelvisPLY:
    ...
    def find_lm_and_align(self, right_pelvis, pel_landmarks, align_fold):
        tm_1 = self.init_align(right_pelvis, pel_landmarks)
        self.find_landmarks(right_pelvis)
        tm_2 = self.align_ISB(right_pelvis)
        self.find_landmarks(right_pelvis)
        tm_3 = self.align_ISB(right_pelvis)
        tm_pelvis = tm_3.dot(tm_2).dot(tm_1)
        self.transform_landmarks(tm_3)
        right_pelvis.transform_landmarks(tm_3)
        self.length_measurements(right_pelvis)
        self.update_acs(right_pelvis)
        self.save_ply(align_fold)
        right_pelvis.save_ply(align_fold)
        return tm_pelvis

    def load_landmark_lists(self, lists_path):
        self.vertex_lists["HJC"] = node("HJC_hemi", lists_path)
        self.vertex_lists["ASIS"] = node("ASIS", lists_path)
        self.vertex_lists["PSIS"] = node("PSIS", lists_path)
        return
    ...
    def find_landmarks(self, Right_pelvis):
        # ASIS
        '''
        Find the point that is the most +ve in the x direction for the left and right side
        '''
        rasis = Right_pelvis.vertices[self.vertex_lists["ASIS"]]
        lasis = self.vertices[self.vertex_lists["ASIS"]]
        i = rasis.argmax(axis=0)[0]
        j = lasis.argmax(axis=0)[0]

        Right_pelvis.landmarks["RASIS"] = {}
        Right_pelvis.landmarks["RASIS"]["ID"] = Right_pelvis.vertex_lists["ASIS"][i]
        Right_pelvis.landmarks["RASIS"]["coords"] = rasis[i, :]

        self.landmarks["LASIS"] = {}
        self.landmarks["LASIS"]["ID"] = self.vertex_lists["ASIS"][j]
        self.landmarks["LASIS"]["coords"] = lasis[j, :]

        mid_point = (Right_pelvis.landmarks["RASIS"]["coords"] + self.landmarks["LASIS"]["coords"]) / 2
        ...
        Find the point that is the most -ve in the x direction for the left and right side
        '''
        rpsis = Right_pelvis.vertices[Right_pelvis.vertex_lists["PSIS"]]
        lpsis = self.vertices[self.vertex_lists["PSIS"]]
        i = rpsis.argmin(axis=0)[0]
        j = lpsis.argmin(axis=0)[0]

        Right_pelvis.landmarks["RPSIS"] = {}
        Right_pelvis.landmarks["RPSIS"]["ID"] = Right_pelvis.vertex_lists["PSIS"][i]
        Right_pelvis.landmarks["RPSIS"]["coords"] = rpsis[i, :]

        self.landmarks["LPSIS"] = {}
        self.landmarks["LPSIS"]["ID"] = self.vertex_lists["PSIS"][j]
        self.landmarks["LPSIS"]["coords"] = lpsis[j, :]

        mid_point = (Right_pelvis.landmarks["RPSIS"]["coords"] + self.landmarks["LPSIS"]["coords"]) / 2
        ...
        '''
        hip_joint = Right_pelvis.vertices[Right_pelvis.vertex_lists["HJC"]]
        ...
        Right_pelvis.landmarks["lengths"] = {}
        Right_pelvis.landmarks["RHJC"] = {}
        Right_pelvis.landmarks["RHJC"]["radius"] = rad
        Right_pelvis.landmarks["lengths"]["RHJ_diameter"] = 2 * rad
        Right_pelvis.landmarks["RHJC"]["coords"] = centre

        left_hip_joint = self.vertices[self.vertex_lists["HJC"]]
    ...
    def find_landmarks_hemi(self, side):
        #finds landmarks only for one hemisphere of the pelvis
        # ASIS
        '''
        Find the point that is the most +ve in the x direction for the left and right side
        '''
        asis = self.vertices[self.vertex_lists["ASIS"]]
        i = asis.argmax(axis=0)[0]

        if side == "Left":
            self.landmarks["LASIS"] = {}
            self.landmarks["LASIS"]["ID"] = self.vertex_lists["ASIS"][i]
            self.landmarks["LASIS"]["coords"] = asis[i, :]
        else:
            self.landmarks["RASIS"] = {}
            self.landmarks["RASIS"]["ID"] = self.vertex_lists["ASIS"][i]
            self.landmarks["RASIS"]["coords"] = asis[i, :]

        # PSIS
        '''
        Find the point that is the most -ve in the x direction for the left and right side
        '''
        psis = self.vertices[self.vertex_lists["PSIS"]]
        i = psis.argmin(axis=0)[0]

        if side == 'Left':
            self.landmarks["LPSIS"] = {}
            self.landmarks["LPSIS"]["ID"] = self.vertex_lists["PSIS"][i]
            self.landmarks["LPSIS"]["coords"] = psis[i, :]
        else:
            self.landmarks["RPSIS"] = {}
            self.landmarks["RPSIS"]["ID"] = self.vertex_lists["PSIS"][i]
            self.landmarks["RPSIS"]["coords"] = psis[i, :]

        # hip joint centre
        '''
        Find the centre of a sphere fit to the acetabulum of the pelvis
        '''
        hip_joint = self.vertices[self.vertex_lists["HJC"]]
    ...
    def length_measurements(self, Right_pelvis):
        # ASIS width
        squared_dist = np.sum((Right_pelvis.landmarks["RASIS"]["coords"] - self.landmarks["LASIS"]["coords"]) ** 2, axis=0)
        length = np.sqrt(squared_dist)
        self.landmarks["lengths"]["ASIS_width"] = length

        # PSIS width
        squared_dist = np.sum((Right_pelvis.landmarks["RPSIS"]["coords"] - self.landmarks["LPSIS"]["coords"]) ** 2, axis=0)
        length = np.sqrt(squared_dist)
    ...
    def init_align(self, Right_pelvis, pel_landmarks):
        # Define eigenvectors
        ...
        RASIS = np.asarray(Right_pelvis.vertices)[pel_landmarks[0], :]
        LPSIS = np.asarray(self.vertices)[pel_landmarks[1], :]
        RPSIS = np.asarray(Right_pelvis.vertices)[pel_landmarks[1], :]
        ...
        Right_pelvis.vertices = Transform.by_tm(Right_pelvis.vertices, tm)
        return tm
    ...
    def align_ISB(self, Right_pelvis):
        ...
        Right_pelvis.vertices = transform3D.transformAffine(Right_pelvis.vertices,tm)

        return tm

    def update_acs(self, Right_pelvis):
        '''
...
class FemurPLY:
    ...
    def find_lm_and_align(self,fem_landmarks,pelvis,align_fold):
        tm_1 = self.init_align_ISB(fem_landmarks)
        tm_2 = self.find_landmarks()
        tm_3 = self.align_ISB(pelvis)
        tm_4 = self.find_landmarks()
        tm_5 = self.align_ISB(pelvis)
        tm_femur = tm_5.dot(tm_4).dot(tm_3).dot(tm_2).dot(tm_1)
        self.transform_landmarks(tm_5)
        self.measurements()
        self.update_acs()
        self.save_ply(align_fold)
        return tm_femur
...
class TibfibPLY:
    ...
    def find_lm_and_align(self,tib_landmarks,femur,align_fold):
        tm_1 = self.init_align_ISB(tib_landmarks)
        self.find_landmarks()
        tm_2 = self.align_ISB(femur)
        self.find_landmarks()
        tm_3 = self.align_ISB(femur)
        self.transform_landmarks(tm_3)
        self.update_acs()
        tm_4 = self.reset_knee_gap(femur)
        self.transform_landmarks(tm_4)
        self.update_acs()
        tm_5 = self.reset_knee_gap_2(femur)
        self.transform_landmarks(tm_5)
        self.update_acs()
        tm_tibfib = tm_5.dot(tm_4).dot(tm_3).dot(tm_2).dot(tm_1)
        self.measurements()
        self.save_ply(align_fold)
        return tm_tibfib
