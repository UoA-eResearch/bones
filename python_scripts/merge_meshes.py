...
aligned_fold = os.path.join(root_dir,'aligned_meshes_both')
        ...
        if case+'Right_femur_rbfreg_rigidreg.ply' in sorted(femur_r):
            if case+'Left_tibfib_rbfreg_rigidreg.ply' in sorted(tibfib_l):
                if case + 'Right_tibfib_rbfreg_rigidreg.ply' in sorted(tibfib_r):
                    ...
                    merged_mesh_vert = np.concatenate((pel_mesh.v,fem_mesh_l.v,tib_mesh_l.v,fem_mesh_r.v,tib_mesh_r.v),axis=0)
                    ...
                    merged_mesh_faces = np.concatenate((pel_mesh.f,fem_mesh_l.f+a,tib_mesh_l.f+a+b,fem_mesh_r.f+a+b+c,tib_mesh_r.f+a+b+c+d),axis=0)
                    #assign vertices and faces to new mesh
                    merged_mesh.v = merged_mesh_vert
                    merged_mesh.f = merged_mesh_faces
                    #save merged mesh
                    save_path = os.path.join(aligned_fold, case+'both_sides')
